...
cookies = {
    ...
    'Hm_lpvt_98beee57fd2ef70ccdd5ca52b9740c49': '1745303562',
...
async def insert_data(cursor, excerpt, content, created_time, author_id, voteup_count):
    try:
        sql = "INSERT INTO zhihu_aio1 (excerpt, content, created_time, author_id, voteup_count) VALUES (%s, %s, %s, %s, %s)"
        await cursor.execute(sql, (str(excerpt), str(content), str(parse_time(created_time)), str(author_id), int(voteup_count)))
    except Exception as e:
        print(f"Error inserting data: {e}")

async def request_index(session, cursor):
    async with session.get(
        ...
        for item in datas:
            ...
            author_id = item['target']['id']
            voteup_count = item['target']['voteup_count']
            await insert_data(cursor, excerpt, content, created_time, author_id, voteup_count)
        return next_url

async def request_qs(session, cursor, index):
    ...
    async with session.get(index, headers=headers, cookies=cookies) as response:
        data = await response.json()
        datas = data['data']
        for item in datas:
            excerpt = item['target'].get('excerpt', '')
            content = item['target'].get('content', '')
            created_time = item['target'].get('created_time', None)
            author_id = item['target'].get('id', None)
            voteup_count = item['target'].get('voteup_count', 0)
            if content and created_time and author_id is not None:
                await insert_data(cursor, excerpt, content, created_time, author_id, voteup_count)
        next_url = data['paging']['next']
        if next_url:
            await request_qs(session, cursor, next_url)

async def parse_time(timestamp):
...
async def main():
    ...
    async with conn.cursor() as cursor:
        ...
        async with aiohttp.ClientSession() as session:
            index = await request_index(session, cursor)
            await request_qs(session, cursor, index)
