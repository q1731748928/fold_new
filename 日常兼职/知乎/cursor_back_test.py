import os
import sys
sys.setrecursionlimit(50000) #例如这里设置为一万
...
def request_index():
    ...
    for data in datas:
        ...
        try:
            sql = "INSERT INTO zhihu_aio1 (cursor_, excerpt, content, created_time, author_id, voteup_count) VALUES ( %s, %s, %s, %s, %s, %s)"
            val = (
            str(cursor_), str(excerpt), str(content), str(parse_time(created_time)), str(author_id), int(voteup_count))
    ...
    for data in datas:
        ...
        if content and created_time and author_id is not None:
            try:
                ...
                # 将处理过的数据插入数据库
                sql = "INSERT INTO zhihu_aio1 (cursor_, excerpt, content, created_time, author_id, voteup_count) VALUES ( %s, %s, %s, %s, %s, %s)"
                val = (str(cursor_), str(excerpt), str(content), str(parse_time(created_time)), str(author_id), int(voteup_count))
                cursor.execute(sql, val)
                db.commit()
            except Exception as e:
                print(f"Error: {e}")
