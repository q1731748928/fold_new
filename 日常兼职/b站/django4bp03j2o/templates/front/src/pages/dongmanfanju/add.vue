...
<div :style='{"width":"1200px","padding":"20px","margin":"10px auto 20px","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="图片" v-if="type!='cross' || (type=='cross' && !ro.cover)" prop="cover">
            <file-upload
            tip="点击上传图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' class="upload" v-else label="图片" prop="cover">
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="徽章" prop="badge">
            <el-input v-model="ruleForm.badge" 
                placeholder="徽章" clearable :disabled=" false  ||ro.badge"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="子标题" prop="subtitle">
            <el-input v-model="ruleForm.subtitle" 
                placeholder="子标题" clearable :disabled=" false  ||ro.subtitle"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="评分" prop="score">
			<el-input-number v-model="ruleForm.score" placeholder="评分" :disabled="ro.score"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="话数" prop="huashu">
            <el-input v-model="ruleForm.huashu" 
                placeholder="话数" clearable :disabled=" false  ||ro.huashu"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="追番" prop="zhuifan">
            <el-input v-model="ruleForm.zhuifan" 
                placeholder="追番" clearable :disabled=" false  ||ro.zhuifan"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="媒体ID" prop="mediaid">
            <el-input v-model="ruleForm.mediaid" 
                placeholder="媒体ID" clearable :disabled=" false  ||ro.mediaid"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="详情地址" prop="link">
            <el-input v-model="ruleForm.link" 
                placeholder="详情地址" clearable :disabled=" false  ||ro.link"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`dongmanfanju/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('dongmanfanju/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`dongmanfanju/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
