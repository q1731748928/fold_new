<template>
	<div class="main-content" :style='{"padding":"0px","margin":"0 auto","color":"#333","borderRadius":"0 0 10px 10px","background":"rgba(255,255,255,.0)","width":"96%","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"20px 0 0","overflow":"hidden","color":"#666","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 30px 30px 0","alignItems":"center","borderRadius":"8px","flexWrap":"wrap","background":"rgba(251,249,243,.9)","display":"flex","width":"auto","fontSize":"18px","justifyContent":"center"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						...
						<el-input v-model="searchForm.genre" placeholder="类型" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">导演</label>
						<el-input v-model="searchForm.directedby" placeholder="导演" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">主演</label>
						<el-input v-model="searchForm.starring" placeholder="主演" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						...
						<el-input v-model="searchForm.lang" placeholder="语言" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 0 30px","color":"#666","alignItems":"center","display":"flex","-webkitPerspective":"320px","justifyContent":"center","borderRadius":"8px","flexWrap":"wrap","flex":"1","background":"rgba(251,249,243,.9)","width":"auto","perspective":"320px","fontSize":"18px"}'>
					...
					<el-button class="btn18" v-if="isAuth('dongmanxinxi','爬取数据')" type="success" @click="spider()">
						<span class="icon iconfont icon-shuju17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						爬取数据
					</el-button>

					<el-button class="btn18" v-if="isAuth('dongmanxinxi','类型统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						类型统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('dongmanxinxi','集数统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						集数统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('dongmanxinxi','评分统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						评分统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('dongmanxinxi','标题')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						标题
					</el-button>
					<el-button class="btn18" v-if="isAuth('dongmanxinxi','语言统计')" type="success" @click="chartDialog5()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						语言统计
					</el-button>
				</el-row>
			...
			<div :style='{"padding":"0 30px","borderColor":"#eee","margin":"30px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.69)","borderWidth":"0px 0 0 0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="initialreleasedate"
						label="首播">
						<template slot-scope="scope">
							{{scope.row.initialreleasedate}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jishu"
		...
		<el-dialog
		  ...
		  width="800">
			<div id="genreChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="jishuChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="pingfenChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="titleChart4" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="langChart5" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var genreChart1 = echarts.init(document.getElementById("genreChart1"),'caravan');
        this.$http({
            url: "dongmanxinxi/group/genre",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var jishuChart2 = echarts.init(document.getElementById("jishuChart2"),'caravan');
        this.$http({
            url: "dongmanxinxi/group/jishu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        var pingfenChart3 = echarts.init(document.getElementById("pingfenChart3"),'caravan');
        this.$http({
            url: "dongmanxinxi/group/pingfen",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                pingfenChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    pingfenChart3.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog4() {
      ...
      this.$nextTick(()=>{
        ...
        var titleChart4 = echarts.init(document.getElementById("titleChart4"),'caravan');
        this.$http({
            url: "dongmanxinxi/group/title",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog5() {
      ...
      this.$nextTick(()=>{
        ...
        var langChart5 = echarts.init(document.getElementById("langChart5"),'caravan');
        this.$http({
            url: "dongmanxinxi/group/lang",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.genre!='' && this.searchForm.genre!=undefined){
            params['genre'] = '%' + this.searchForm.genre + '%'
          }
           if(this.searchForm.directedby!='' && this.searchForm.directedby!=undefined){
            params['directedby'] = '%' + this.searchForm.directedby + '%'
          }
           if(this.searchForm.starring!='' && this.searchForm.starring!=undefined){
            params['starring'] = '%' + this.searchForm.starring + '%'
          }
           if(this.searchForm.lang!='' && this.searchForm.lang!=undefined){
            params['lang'] = '%' + this.searchForm.lang + '%'
          }
